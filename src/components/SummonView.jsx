      ...
      <div style={{ background: '#0f172a', padding: '15px', borderRadius: '8px', border: '1px solid #334155', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        <div style={{ width: '100%', marginBottom: '15px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '10px', color: '#eab308', marginBottom: '2px' }}>
            <span>Légendaire Garanti</span><span>{pity?.legendary ?? 0}/100</span>
          </div>
          <div className="pity-bar-container" style={{ width: '100%', background: '#334155', borderRadius: '5px', overflow: 'hidden', height: '6px', marginBottom: '8px' }}>
            <div className="pity-bar-fill" style={{ width: `${Math.min(100, ((pity?.legendary ?? 0) / 100) * 100)}%`, background: '#eab308', height: '100%', transition: 'width 0.3s' }}></div>
          </div>

          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '10px', color: '#ef4444', marginBottom: '2px' }}>
            <span>Mythique Garanti</span><span>{pity?.mythic ?? 0}/500</span>
          </div>
          <div className="pity-bar-container" style={{ width: '100%', background: '#334155', borderRadius: '5px', overflow: 'hidden', height: '6px', marginBottom: '8px' }}>
            <div className="pity-bar-fill" style={{ width: `${Math.min(100, ((pity?.mythic ?? 0) / 500) * 100)}%`, background: '#ef4444', height: '100%', transition: 'width 0.3s' }}></div>
          </div>

          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '10px', color: '#06b6d4', marginBottom: '2px' }}>
            <span>Ultra Garanti</span><span>{pity?.ultra ?? 0}/1500</span>
          </div>
          <div className="pity-bar-container" style={{ width: '100%', background: '#334155', borderRadius: '5px', overflow: 'hidden', height: '6px' }}>
            <div className="pity-bar-fill" style={{ width: `${Math.min(100, ((pity?.ultra ?? 0) / 1500) * 100)}%`, background: '#06b6d4', height: '100%', transition: 'width 0.3s' }}></div>
          </div>
        </div>

        <div style={{ display: 'flex', gap: '10px', width: '100%', marginBottom: '10px' }}>
          <button className="summon-btn" onClick={() => performSummon(1)} disabled={(activeBanner === 'premium' ? res.gems < baseCost : res.gold < baseCost)} style={{ flex: 1, padding: '15px', fontSize: '16px', fontWeight: 'bold' }}>
